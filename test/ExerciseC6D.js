...
contract('ExerciseC6D', async (accounts) => {
  ...
  it('can register oracles', async () => {
    ...
    for(let a=1; a<TEST_ORACLES_COUNT; a++) {      
      ...
      let result = await config.exerciseC6D.getOracle.call(accounts[a]); //not in initially
      console.log('Oracle Registered:${result[0]}, ${result[1]}, ${result[2]}', result[0], result[1], result[2]);
    }
  });
